...
/**
 * Assigns a teacher to a class, replacing whatever class they were
 * previously assigned to (rather than just adding another one). A
 * teacher must always have exactly one current class — the previous
 * version of this endpoint only ever added assignments, never retired
 * the old one, so a reassigned teacher would end up with more than one
 * "active" class at once and the app had no reliable way to know which
 * was actually current (e.g. the create-spreadsheet wizard, which
 * expects exactly one).
 */
async function assignTeacherToClass(req, res) {
  ...
  const assignment = await prisma.$transaction(async (tx) => {
    // Retire every other currently-active assignment this teacher has —
    // historical spreadsheets built under an old assignment are
    // completely unaffected, since those reference the Class directly,
    // not this join row.
    await tx.classAssignment.updateMany({
      where: { teacherId, removedAt: null, NOT: { classId } },
      data: { removedAt: new Date() },
    });

    return tx.classAssignment.upsert({
      where: { teacherId_classId: { teacherId, classId } },
      update: { removedAt: null, isPrimary: true },
