...
function route() {
  ...
  switch (parts[0]) {
    ...
    case "announcements": return parts[1] ? renderShell(() => renderSentAnnouncementDetail(parts[1], "/admin", "#/announcements")) : renderShell(() => renderAnnouncementsHub("/admin", "#/dashboard"));
    case "calendar": return renderShell(() => renderCalendarView({ apiPrefix: "/admin", canEdit: true, backHash: "#/dashboard" }));
    // ---- Teacher ----
    ...
    case "teacher-mailbox": return parts[1] ? renderTeacherShell(() => renderMailboxDetail(parts[1], "/teacher", "#/teacher-mailbox")) : renderTeacherShell(() => renderMailboxList("/teacher", "#/teacher-dashboard"));
    case "teacher-announcements": return parts[1] ? renderTeacherShell(() => renderSentAnnouncementDetail(parts[1], "/teacher", "#/teacher-announcements")) : renderTeacherShell(() => renderAnnouncementsHub("/teacher", "#/teacher-dashboard"));
    case "teacher-calendar": return renderTeacherShell(() => renderCalendarView({ apiPrefix: "/teacher", canEdit: false, backHash: "#/teacher-dashboard" }));
    ...
    case "student-mailbox": return parts[1] ? renderStudentShell(() => renderMailboxDetail(parts[1], "/student", "#/student-mailbox")) : renderStudentShell(() => renderMailboxList("/student", "#/student-dashboard"));
    case "student-calendar": return renderStudentShell(() => renderCalendarView({ apiPrefix: "/student", canEdit: false, backHash: "#/student-dashboard" }));
...
async function renderDashboard(contentEl) {
  const [summary, teachers, students, todayEvents] = await Promise.all([
    api("/admin/dashboard-summary"),
    api("/admin/teachers"),
    api("/admin/students"),
    api("/admin/calendar-events/today").catch(() => ({ events: [] })),
  ]);

  contentEl.innerHTML = `
    ${todayReminderBannerHtml(todayEvents.events)}
    ...
    <div class="tile-grid">
      ...
      <div class="tile" data-go="#/announcements"><span class="tile-icon">📢</span>Announcements<span class="tile-count">Message all</span></div>
      <div class="tile" data-go="#/calendar"><span class="tile-icon">📅</span>Calendar<span class="tile-count">School events</span></div>
...
async function renderTeacherProfile(teacherId) {
  ...
  const currentClassId = t.classes[0]?.id || "";
    ...
    <div class="card">
      <div class="form-group" style="margin-bottom:10px;">
        ...
        <select id="assign-class-select">
          ...
          ${classesData.classes.map((c) => `<option value="${c.id}" ${currentClassId === c.id ? "selected" : ""}>${escapeHtml(c.name)}</option>`).join("")}
        </select>
      </div>
  ...
  document.getElementById("assign-class-btn").addEventListener("click", async () => {
    ...
    if (classId === currentClassId) { alertEl.innerHTML = `<div class="alert alert-info">That's already this teacher's class.</div>`; return; }
    try {
      await api("/admin/classes/assign-teacher", { method: "POST", body: { teacherId: t.id, classId } });
...
async function renderTeacherShell(contentRenderer) {
  ...
  appEl.innerHTML = `
    ...
    <div class="bottom-nav">
      ...
      <button class="nav-item ${activePath === "teacher-mailbox" ? "active" : ""}" data-go="#/teacher-mailbox"><span class="nav-icon">📬</span>Mail</button>
      <button class="nav-item ${activePath === "teacher-calendar" ? "active" : ""}" data-go="#/teacher-calendar"><span class="nav-icon">📅</span>Calendar</button>
...
async function renderTeacherDashboard(contentEl) {
  const [me, sheets, todayEvents] = await Promise.all([
    api("/teacher/me"),
    api("/teacher/spreadsheets"),
    api("/teacher/calendar-events/today").catch(() => ({ events: [] })),
  ]);
  const t = me.teacher;
...
async function renderCreateSpreadsheetWizard(contentEl) {
  ...
  document.getElementById("create-submit-btn").addEventListener("click", async () => {
    ...
    try {
      ...
      if (err.data?.spreadsheetId) {
        alertEl.innerHTML = `<div class="alert alert-error">${err.message} If you'd like to start fresh, open it and delete it first (only possible if it's never been locked).</div>`;
      } else {
        alertEl.innerHTML = `<div class="alert alert-error">${err.message}</div>`;
      }
    }
...
async function renderSpreadsheetEditor(versionId) {
  ...
  const spreadsheetDetail = await api(`/teacher/spreadsheets/${view.spreadsheet.id}`).catch(() => ({ allVersions: [] }));
  const everLocked = spreadsheetDetail.allVersions.some((v) => v.status === "LOCKED");
  ...
  contentEl.innerHTML = `
    ...
    ${!locked ? `
      <div class="btn-row" style="margin-bottom:10px;">
        <button class="btn btn-outline btn-sm" id="add-student-btn">➕ Add Student</button>
        <button class="btn btn-outline btn-sm" id="add-subject-btn">➕ Add Subject</button>
        ${!everLocked ? `<button class="btn btn-danger btn-sm" id="delete-spreadsheet-btn">🗑️ Delete Spreadsheet</button>` : ""}
      </div>
    ` : ""}
  ...
  function renderTable() {
    ...
    const subjectHeaderCells = view.subjects.map((s) => `<th colspan="4">${escapeHtml(s.name)}${!locked ? ` <button type="button" class="back-btn" style="width:18px; height:18px; font-size:10px; margin-left:4px; background:rgba(255,255,255,0.25); color:#fff;" data-remove-subject="${s.id}" aria-label="Remove ${escapeHtml(s.name)} from this spreadsheet" title="Remove subject">✕</button>` : ""}</th>`).join("");
        ...
        ${view.rows.map((row, idx) => {
          ...
          return `
            <tr>
              <td>${sn}</td>
              <td class="name-cell">
                ${escapeHtml(row.student.firstName)} ${escapeHtml(row.student.lastName)}
                ${!locked ? `<button type="button" class="back-btn" style="width:22px; height:22px; font-size:11px; margin-left:6px;" data-remove-student="${row.student.id}" aria-label="Remove ${escapeHtml(row.student.firstName)} from this spreadsheet" title="Remove from spreadsheet">✕</button>` : ""}
              </td>
              ${subjectCells}
  ...
  function bindEditorEvents() {
    contentEl.querySelectorAll("[data-remove-student]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const studentId = btn.dataset.removeStudent;
        const row = view.rows.find((r) => r.student.id === studentId);
        showConfirmModal({
          title: "Remove this student?",
          message: `Remove ${row ? row.student.firstName + " " + row.student.lastName : "this student"} from this spreadsheet? Any scores already entered for them here will be deleted too.`,
          confirmLabel: "Remove",
          danger: true,
          onConfirm: async () => {
            try {
              await api(`/teacher/spreadsheet-versions/${versionId}/students/${studentId}`, { method: "DELETE" });
    ...
    contentEl.querySelectorAll("[data-remove-subject]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        const subjectId = btn.dataset.removeSubject;
        const subject = view.subjects.find((s) => s.id === subjectId);
        showConfirmModal({
          title: "Remove this subject?",
          message: `Remove ${subject ? subject.name : "this subject"} from this spreadsheet? Every student's score already entered for it here will be deleted too.`,
          confirmLabel: "Remove",
          danger: true,
          onConfirm: async () => {
            try {
              await api(`/teacher/spreadsheet-versions/${versionId}/subjects/${subjectId}`, { method: "DELETE" });
              renderSpreadsheetEditor(versionId);
            } catch (err) {
              alert(err.message);
            }
  ...
  if (addStudentBtn) {
    addStudentBtn.addEventListener("click", async () => {
      const options = await api("/teacher/create-options");
      const onRosterIds = new Set(view.rows.map((r) => r.student.id));
      const candidates = options.students.filter((s) => !onRosterIds.has(s.id));
      ...
      wrap.innerHTML = `
        <div class="modal-sheet">
          <h3>Add Students</h3>
          ${candidates.length ? `
            <div class="checkbox-list" style="max-height:260px;">
              ${candidates.map((s) => `<label class="checkbox-row"><input type="checkbox" class="add-student-cb" value="${s.id}" /> ${escapeHtml(s.firstName)} ${escapeHtml(s.lastName)}</label>`).join("")}
            </div>
          ` : `<div class="empty-state">Every verified student in your class is already on this spreadsheet.</div>`}
          <div id="add-student-alert" style="margin-top:10px;"></div>
          <div class="btn-row">
            <button class="btn btn-outline" id="add-student-cancel">Cancel</button>
            ${candidates.length ? `<button class="btn btn-primary" id="add-student-confirm">Add Selected</button>` : ""}
          </div>
        </div>`;
      document.body.appendChild(wrap);
      wrap.querySelector("#add-student-cancel").addEventListener("click", () => wrap.remove());
      const confirmBtn = wrap.querySelector("#add-student-confirm");
      if (confirmBtn) {
        confirmBtn.addEventListener("click", async () => {
          const studentIds = Array.from(wrap.querySelectorAll(".add-student-cb:checked")).map((cb) => cb.value);
          if (!studentIds.length) { wrap.querySelector("#add-student-alert").innerHTML = `<div class="alert alert-error">Select at least one student.</div>`; return; }
          try {
            await api(`/teacher/spreadsheet-versions/${versionId}/students`, { method: "POST", body: { studentIds } });
            wrap.remove();
            renderSpreadsheetEditor(versionId);
          } catch (err) {
            wrap.querySelector("#add-student-alert").innerHTML = `<div class="alert alert-error">${err.message}</div>`;
          }
        });
      }
    });
  ...
  if (addSubjectBtn) {
    addSubjectBtn.addEventListener("click", async () => {
      const options = await api("/teacher/create-options");
      const onSheetIds = new Set(view.subjects.map((s) => s.id));
      let candidates = options.subjects.filter((s) => !onSheetIds.has(s.id));

      const wrap = document.createElement("div");
      wrap.className = "modal-backdrop";
      const renderList = () => `
        ${candidates.length ? `
          <div class="checkbox-list" style="max-height:220px;">
            ${candidates.map((s) => `<label class="checkbox-row"><input type="checkbox" class="add-subject-cb" value="${s.id}" /> ${escapeHtml(s.name)}</label>`).join("")}
          </div>
        ` : `<div class="empty-state">Every existing subject is already on this spreadsheet.</div>`}
      `;
      wrap.innerHTML = `
        <div class="modal-sheet">
          <h3>Add Subjects</h3>
          <div id="add-subject-list">${renderList()}</div>
          <div style="display:flex; gap:8px; margin:12px 0;">
            <input type="text" id="new-subject-name" placeholder="Or add a new subject…" style="flex:1; min-height:44px; border-radius:10px; border:1.5px solid var(--gray-200); padding:0 14px;" />
            <button class="btn btn-outline btn-sm" id="create-new-subject-btn">Add</button>
          </div>
          <div id="add-subject-alert"></div>
          <div class="btn-row">
            <button class="btn btn-outline" id="add-subject-cancel">Cancel</button>
            <button class="btn btn-primary" id="add-subject-confirm">Add Selected</button>
          </div>
        </div>`;
      document.body.appendChild(wrap);
      wrap.querySelector("#add-subject-cancel").addEventListener("click", () => wrap.remove());

      wrap.querySelector("#create-new-subject-btn").addEventListener("click", async () => {
        const alertEl = wrap.querySelector("#add-subject-alert");
        const name = wrap.querySelector("#new-subject-name").value.trim();
        if (!name) return;
        try {
          const data = await api("/teacher/subjects", { method: "POST", body: { name } });
          if (!onSheetIds.has(data.subject.id) && !candidates.find((c) => c.id === data.subject.id)) candidates.push(data.subject);
          wrap.querySelector("#add-subject-list").innerHTML = renderList();
          wrap.querySelector(`.add-subject-cb[value="${data.subject.id}"]`).checked = true;
          wrap.querySelector("#new-subject-name").value = "";
        } catch (err) {
          alertEl.innerHTML = `<div class="alert alert-error">${err.message}</div>`;
        }
      });
      ...
      wrap.addEventListener("click", async (e) => {
        if (e.target.id !== "add-subject-confirm") return;
        const subjectIds = Array.from(wrap.querySelectorAll(".add-subject-cb:checked")).map((cb) => cb.value);
        if (!subjectIds.length) { wrap.querySelector("#add-subject-alert").innerHTML = `<div class="alert alert-error">Select at least one subject.</div>`; return; }
        try {
          await api(`/teacher/spreadsheet-versions/${versionId}/subjects`, { method: "POST", body: { subjectIds } });
          wrap.remove();
          renderSpreadsheetEditor(versionId);
        } catch (err) {
          wrap.querySelector("#add-subject-alert").innerHTML = `<div class="alert alert-error">${err.message}</div>`;
        }
      });
    });
  }

  const deleteSpreadsheetBtn = document.getElementById("delete-spreadsheet-btn");
  if (deleteSpreadsheetBtn) {
    deleteSpreadsheetBtn.addEventListener("click", () => {
      showConfirmModal({
        title: "Delete this spreadsheet?",
        message: "This permanently deletes the entire spreadsheet and everything in it — every student, score, and remark. This cannot be undone. This is only possible because it has never been locked.",
        confirmLabel: "Delete Permanently",
        danger: true,
        onConfirm: async () => {
          try {
            await api(`/teacher/spreadsheets/${view.spreadsheet.id}`, { method: "DELETE" });
            go("#/teacher-dashboard");
          } catch (err) {
            alert(err.message);
          }
...
async function renderStudentShell(contentRenderer) {
  ...
  appEl.innerHTML = `
    ...
    <div class="bottom-nav">
      ...
      <button class="nav-item ${activePath === "student-mailbox" ? "active" : ""}" data-go="#/student-mailbox"><span class="nav-icon">📬</span>Mail</button>
      <button class="nav-item ${activePath === "student-calendar" ? "active" : ""}" data-go="#/student-calendar"><span class="nav-icon">📅</span>Calendar</button>
...
async function renderStudentDashboard(contentEl) {
  const [me, schoolInfo, reportCards, todayEvents] = await Promise.all([
    api("/student/me"),
    fetch("/api/public/school-info").then((r) => r.json()).catch(() => ({ schoolName: "ZICHRI SCHOOL", address: "" })),
    api("/student/report-cards"),
    api("/student/calendar-events/today").catch(() => ({ events: [] })),
  ]);
  const s = me.student;
...
// ============================================================================
// ANNOUNCEMENTS (Admin + Teacher compose; shared "sent" read-receipts view)
// ============================================================================

async function renderAnnouncementsHub(apiPrefix, backHash) {
  const contentEl = document.getElementById("content");
  const isAdmin = apiPrefix === "/admin";

  let classesData = { classes: [] };
  if (isAdmin) classesData = await api("/admin/classes");

  contentEl.innerHTML = `
    ${pageHeaderHtml(isAdmin ? "Announcements" : "Message My Class", backHash)}
    <div class="tabs" id="ann-tabs">
      <button class="tab-btn active" data-tab="compose">Compose</button>
      <button class="tab-btn" data-tab="sent">Sent</button>
    </div>
    <div id="ann-tab-content"></div>
  `;
  bindNav(contentEl);

  function renderComposeTab() {
    const tabContent = document.getElementById("ann-tab-content");
    tabContent.innerHTML = `
      <div class="card">
        <div class="form-group">
          <label>Message</label>
          <textarea id="ann-message" rows="5" placeholder="Type your message…"></textarea>
        </div>
        <div class="form-group">
          <label>Attach files (optional — PDF, Word, or images, up to 5)</label>
          <input type="file" id="ann-attachments" multiple accept=".pdf,.doc,.docx,image/jpeg,image/png,image/webp" />
        </div>
        ${isAdmin ? `
          <div class="form-group">
            <label style="display:flex; align-items:center; gap:8px;">
              <input type="checkbox" id="ann-send-all" style="width:20px; height:20px; accent-color:var(--blue-600);" /> Send to ALL classes
            </label>
          </div>
          <div id="ann-class-picker-wrap">
            <label style="display:block; font-size:13px; font-weight:600; color:var(--gray-700); margin-bottom:6px;">Or choose specific classes</label>
            <div class="class-picker-list">
              ${classesData.classes.length ? classesData.classes.map((c) => `
                <label class="class-picker-row"><input type="checkbox" class="ann-class-cb" value="${c.id}" /> ${escapeHtml(c.name)}</label>
              `).join("") : `<div class="empty-state">No classes exist yet.</div>`}
            </div>
          </div>
        ` : `<div class="alert alert-info">This will be sent only to the students in your own class.</div>`}
        <div id="ann-alert"></div>
        <button class="btn btn-primary" id="ann-send-btn">Send Message</button>
      </div>
    `;

    const sendAllCb = document.getElementById("ann-send-all");
    if (sendAllCb) {
      sendAllCb.addEventListener("change", () => {
        document.getElementById("ann-class-picker-wrap").style.display = sendAllCb.checked ? "none" : "block";
      });
    }

    document.getElementById("ann-send-btn").addEventListener("click", () => {
      const alertEl = document.getElementById("ann-alert");
      const message = document.getElementById("ann-message").value.trim();
      if (!message) { alertEl.innerHTML = `<div class="alert alert-error">Write a message first.</div>`; return; }

      const sendToAll = isAdmin && document.getElementById("ann-send-all").checked;
      const selectedClassIds = isAdmin ? Array.from(contentEl.querySelectorAll(".ann-class-cb:checked")).map((cb) => cb.value) : [];
      if (isAdmin && !sendToAll && !selectedClassIds.length) {
        alertEl.innerHTML = `<div class="alert alert-error">Select at least one class, or check "Send to ALL classes".</div>`;
        return;
      }

      const confirmMessage = isAdmin
        ? (sendToAll
            ? "This will send to EVERY teacher and student across ALL classes in the school. This cannot be undone. Continue?"
            : `This will send to the teacher(s) and every student in: ${selectedClassIds.map((id) => classesData.classes.find((c) => c.id === id)?.name).join(", ")}. Continue?`)
        : "This will send to every student in your class. Continue?";

      showConfirmModal({
        title: "Send this message?",
        message: confirmMessage,
        confirmLabel: "Send",
        danger: isAdmin && sendToAll,
        onConfirm: async () => {
          const form = new FormData();
          form.append("message", message);
          if (isAdmin) {
            form.append("sendToAll", sendToAll ? "true" : "false");
            if (!sendToAll) form.append("classIds", JSON.stringify(selectedClassIds));
          }
          const files = document.getElementById("ann-attachments").files;
          for (const f of files) form.append("attachments", f);

          try {
            const data = await api(`${apiPrefix}/announcements`, { method: "POST", body: form, isForm: true });
            alertEl.innerHTML = `<div class="alert alert-success">Sent to ${data.recipientCount} recipient(s).</div>`;
  ...
  async function renderSentTab() {
    ...
    const data = await api(`${apiPrefix}/announcements`);
    if (!data.announcements.length) { tabContent.innerHTML = `<div class="empty-state">Nothing sent yet.</div>`; return; }
    tabContent.innerHTML = data.announcements.map((a) => `
      <div class="card spreadsheet-card" data-open="${a.id}">
        <div>
          <div class="title">${a.sentToAll ? "Sent to ALL classes" : escapeHtml(a.classes.join(", "))}</div>
          <div class="sub">${escapeHtml(a.message.slice(0, 60))}${a.message.length > 60 ? "…" : ""} · ${fmtDateTime(a.createdAt)}</div>
        </div>
        <span class="version-tag">${a.readCount}/${a.totalRecipients} read</span>
      </div>
    `).join("");
    tabContent.querySelectorAll("[data-open]").forEach((el) => el.addEventListener("click", () => go(`${apiPrefix === "/admin" ? "#/announcements" : "#/teacher-announcements"}/${el.dataset.open}`)));
  }

  contentEl.querySelectorAll("#ann-tabs .tab-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      contentEl.querySelectorAll("#ann-tabs .tab-btn").forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      if (btn.dataset.tab === "compose") renderComposeTab(); else renderSentTab();
    });
  });
...
/** Read-receipts view for one sent message — who has and hasn't opened it. */
async function renderSentAnnouncementDetail(id, apiPrefix, backHash) {
  const contentEl = document.getElementById("content");
  const data = await api(`${apiPrefix}/announcements/${id}`);

  contentEl.innerHTML = `
    ${pageHeaderHtml("Message Details", backHash)}
    <div class="card">
      <div class="mail-detail-body">${escapeHtml(data.message)}</div>
      ${data.attachments.length ? data.attachments.map((a) => `
        <div class="attachment-chip"><span class="att-icon">📎</span><span class="att-name">${escapeHtml(a.name)}</span></div>
      `).join("") : ""}
      <div style="font-size:12px; color:var(--gray-500); margin-top:10px;">
        ${data.sentToAll ? "Sent to ALL classes" : escapeHtml(data.classes.join(", "))} · ${fmtDateTime(data.createdAt)}
      </div>
    </div>

    <div class="section-title">Read by ${data.recipients.filter((r) => r.read).length} of ${data.recipients.length}</div>
    <div class="card">
      ${data.recipients.map((r) => `
        <div class="read-receipt-row">
          <span><span class="rr-name">${escapeHtml(r.name)}</span><span class="rr-role">${r.role}</span></span>
          <span>${r.read ? "✅ Read" : "⬜ Not yet"}</span>
        </div>
      `).join("")}
    </div>
  `;
...
// ============================================================================
// MAILBOX (receiving side — shared by Teacher and Student)
// ============================================================================

async function renderMailboxList(apiPrefix, backHash) {
  const contentEl = document.getElementById("content");
  const data = await api(`${apiPrefix}/mailbox`);
  const routePrefix = apiPrefix === "/teacher" ? "#/teacher-mailbox" : "#/student-mailbox";

  contentEl.innerHTML = `
    ${pageHeaderHtml("Mailbox", backHash)}
    ${data.messages.length ? data.messages.map((m) => `
      <div class="mail-item ${m.read ? "" : "unread"}" data-open="${m.id}">
        <div class="mail-top">
          <div style="display:flex; gap:8px;">
            ${!m.read ? `<div class="unread-dot"></div>` : ""}
            <div class="mail-author">${escapeHtml(m.authorLabel)}</div>
          </div>
          <div class="mail-time">${fmtDateTime(m.createdAt)}</div>
        </div>
        <div class="mail-preview">${escapeHtml(m.message.slice(0, 100))}${m.message.length > 100 ? "…" : ""}</div>
        ${m.attachmentCount ? `<div class="mail-meta-row"><span class="attachment-chip" style="margin:0; padding:4px 10px;">📎 ${m.attachmentCount} attachment${m.attachmentCount > 1 ? "s" : ""}</span></div>` : ""}
      </div>
    `).join("") : `<div class="empty-state">Your mailbox is empty.</div>`}
  `;
  bindNav(contentEl);
  contentEl.querySelectorAll("[data-open]").forEach((el) => el.addEventListener("click", () => go(`${routePrefix}/${el.dataset.open}`)));
}

async function renderMailboxDetail(id, apiPrefix, backHash) {
  const contentEl = document.getElementById("content");
  const data = await api(`${apiPrefix}/mailbox/${id}`);

  const daysLeft = Math.max(0, Math.ceil((new Date(data.expiresAt) - new Date()) / (1000 * 60 * 60 * 24)));

  contentEl.innerHTML = `
    ${pageHeaderHtml("Message", backHash)}
    <div class="expiry-warning">📌 Please make sure to read and download anything you need — this message and its attachments will be permanently removed in ${daysLeft} day${daysLeft === 1 ? "" : "s"}.</div>
    <div class="card">
      <div class="mail-detail-header">
        <div class="mail-author">${escapeHtml(data.authorLabel)}</div>
        <div style="font-size:12px; color:var(--gray-500);">${fmtDateTime(data.createdAt)}</div>
      </div>
      <div class="mail-detail-body">${escapeHtml(data.message)}</div>
      ${data.attachments.length ? data.attachments.map((a) => `
        <div class="attachment-chip" data-download="${a.id}" data-name="${escapeHtml(a.name)}" style="cursor:pointer;">
          <span class="att-icon">📎</span><span class="att-name">${escapeHtml(a.name)}</span>
  ...
  contentEl.querySelectorAll("[data-download]").forEach((el) => {
    el.addEventListener("click", async () => {
      try {
        await downloadAuthenticatedFile(`${apiPrefix}/mailbox/${id}/attachments/${el.dataset.download}`, el.dataset.name || "attachment");
      } catch (err) {
        alert(err.message);
      }
    });
...
// ============================================================================
// CALENDAR (shared by all three roles — Admin can add/remove events)
// ============================================================================
...
/** The in-app "today" reminder — shown on every dashboard when Admin has added an event for today. */
function todayReminderBannerHtml(events) {
  if (!events || !events.length) return "";
  return `
    <div class="today-reminder-banner">
      <span class="icon">🔔</span>
      <span class="text">Today: ${events.map((e) => escapeHtml(e.title)).join(", ")}</span>
    </div>
  `;
}

async function renderCalendarView({ apiPrefix, canEdit, backHash }) {
  const contentEl = document.getElementById("content");
  ...
  contentEl.innerHTML = `
    ${pageHeaderHtml("Calendar", backHash)}
    <div class="floating-date-bar">
      <div class="fdb-segment">
        <div class="fdb-day-name">${FULL_WEEKDAY_NAMES[today.getDay()]}</div>
      </div>
      <div class="fdb-divider"></div>
      <div class="fdb-segment">
        <div class="fdb-day-num">${ordinal(today.getDate())}</div>
      </div>
      <div class="fdb-divider"></div>
      <div class="fdb-segment">
        <div class="fdb-month-year">${MONTH_NAMES[today.getMonth()]} ${today.getFullYear()}</div>
      </div>
    </div>
    ...
    <div class="calendar-event-list-title">Events this month</div>
    <div id="cal-event-list"></div>
  `;
  bindNav(contentEl);

  async function loadMonth() {
    document.getElementById("cal-month-label").textContent = `${MONTH_NAMES[viewMonth - 1]} ${viewYear}`;
    const data = await api(`${apiPrefix}/calendar-events?year=${viewYear}&month=${viewMonth}`);
    const eventsByDay = {};
    ...
    let html = WEEKDAY_NAMES.map((w) => `<div class="calendar-weekday">${w}</div>`).join("");
    for (let i = 0; i < startWeekday; i++) html += `<div class="calendar-day-cell empty"></div>`;
    for (let d = 1; d <= daysInMonth; d++) {
      const isToday = isCurrentMonth && d === today.getDate();
      const hasEvent = !!eventsByDay[d];
      html += `<div class="calendar-day-cell ${isToday ? "today" : ""} ${hasEvent ? "has-event" : ""}" data-day="${d}">
        <span>${d}</span>${hasEvent ? `<span class="event-dot"></span>` : ""}
      </div>`;
    }
    gridEl.innerHTML = html;

    gridEl.querySelectorAll("[data-day]").forEach((cell) => {
      cell.addEventListener("click", () => openDayPanel(Number(cell.dataset.day), eventsByDay[cell.dataset.day] || []));
    });

    const listEl = document.getElementById("cal-event-list");
    listEl.innerHTML = data.events.length ? data.events.map((e) => `
      <div class="calendar-event-row">
        <div class="event-date-chip">${ordinal(new Date(e.date).getUTCDate())}</div>
        <div class="event-info">
          <div class="event-title">${escapeHtml(e.title)}</div>
          ${e.description ? `<div class="event-desc">${escapeHtml(e.description)}</div>` : ""}
        </div>
      </div>
    `).join("") : `<div class="empty-state">No events this month.</div>`;
  }

  function openDayPanel(day, dayEvents) {
    const dateLabel = `${MONTH_NAMES[viewMonth - 1]} ${ordinal(day)}, ${viewYear}`;
    const wrap = document.createElement("div");
    wrap.className = "modal-backdrop";
    wrap.innerHTML = `
      <div class="modal-sheet">
        ...
        <div id="day-events-list">
          ${dayEvents.length ? dayEvents.map((e) => `
            <div class="calendar-event-row">
              ...
              ${canEdit ? `<button class="btn btn-danger btn-sm" data-delete-event="${e.id}">Remove</button>` : ""}
            </div>
          `).join("") : `<div class="empty-state">No events on this day.</div>`}
        </div>
        ${canEdit ? `
          <div class="form-group" style="margin-top:14px;"><label>Add an event</label><input type="text" id="new-event-title" placeholder="e.g. Children's Day" /></div>
          <div class="form-group"><textarea id="new-event-desc" rows="2" placeholder="Optional notes…"></textarea></div>
          <div id="day-panel-alert"></div>
          <button class="btn btn-primary" id="add-event-btn">Add Event</button>
        ` : ""}
        <button class="btn btn-outline" id="day-panel-close" style="margin-top:10px;">Close</button>
      </div>`;
    document.body.appendChild(wrap);

    wrap.querySelector("#day-panel-close").addEventListener("click", () => wrap.remove());
    ...
    wrap.querySelectorAll("[data-delete-event]").forEach((btn) => {
      btn.addEventListener("click", () => {
        showConfirmModal({
          ...
          danger: true,
          onConfirm: async () => {
            await api(`/admin/calendar-events/${btn.dataset.deleteEvent}`, { method: "DELETE" });
            wrap.remove();
            loadMonth();
          },
        });
    ...
    const addBtn = wrap.querySelector("#add-event-btn");
    if (addBtn) {
      addBtn.addEventListener("click", async () => {
        const alertEl = wrap.querySelector("#day-panel-alert");
        const title = wrap.querySelector("#new-event-title").value.trim();
        if (!title) { alertEl.innerHTML = `<div class="alert alert-error">Enter a title first.</div>`; return; }
        const dateStr = `${viewYear}-${String(viewMonth).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
        try {
          await api("/admin/calendar-events", { method: "POST", body: { date: dateStr, title, description: wrap.querySelector("#new-event-desc").value.trim() || undefined } });
          wrap.remove();
          loadMonth();
        } catch (err) {
          alertEl.innerHTML = `<div class="alert alert-error">${err.message}</div>`;
