...
# Post-Launch: Live Deployment on Render + Neon + Brevo

After Phase 7, the app was deployed to production on free-tier hosting
and a series of real-world fixes were made based on what only shows up
once real traffic and a real host are involved. Summarized here for
anyone picking up this project later — full blow-by-blow context lives
in the separate operational reference document kept alongside this repo.

## Stack in production

- **Hosting**: Render (free web service tier)
- **Database**: Neon (free PostgreSQL — `provider = "postgresql"` in
  `schema.prisma` is now permanent; SQLite is no longer supported by this
  project, since it can't represent this schema's enum types at all)
- **Email**: Brevo's HTTPS API (`src/services/mailer.service.js`) — not
  SMTP. Render's free tier blocks all outbound SMTP ports (25/465/587)
  as an anti-spam measure, so raw SMTP (including Gmail's) cannot work
  there regardless of configuration. Brevo sends over ordinary HTTPS,
  which isn't blocked, and has a genuinely free tier.

## File storage

Uploaded profile pictures, the school logo, and generated report card
PDFs are stored as raw bytes directly in the database (`UploadedFile.data`),
not on local disk. This is deliberate: most free hosts (including
Render's free tier) wipe local disk on every restart/redeploy, so
anything written there doesn't survive. Re-uploading a photo or logo now
also deletes the file it replaced, so this doesn't quietly accumulate
storage forever.

## Operational features added after launch

- **Maintenance mode** — setting `MAINTENANCE_MODE=true` in the
  environment shows every visitor a friendly "back soon" page instead of
  the app, with zero code changes or redeploys needed to toggle it.
- **Automated daily backups** — `.github/workflows/backup.yml` runs on
  GitHub's own infrastructure (independent of Render/Neon, so it isn't
  affected if either has a problem), dumps the database daily, and keeps
  a rolling 30 days of history in the `backups/` folder. See
  `backups/README.md` for restore instructions. Requires a `DATABASE_URL`
  repository secret (separate from Render's copy of the same value —
  GitHub Actions can't see Render's environment variables).
- `app.set("trust proxy", 1)` in `server.js` — required for
  express-rate-limit to work correctly behind Render's reverse proxy;
  without it, login/registration would intermittently fail outright with
  `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR`.
- Static frontend files are served with `Cache-Control: no-cache` — every
  page load revalidates with the server rather than trusting a
  potentially stale cached copy, so a deployed update reliably reaches
  people without them needing to manually clear their browser's site data.
- Admin gained the ability to create Academic Sessions/Terms and assign
  or reassign a teacher's or student's class — these existed as backend
  endpoints since early phases but had no frontend screen until a real
  admin tried to use them and found nothing there.
- The dashboard's failed-delivery notification badge only counts
  failures from the last 7 days, rather than accumulating forever — a
  since-resolved problem (e.g. email wasn't configured yet) used to sit
  in that count permanently with no way to clear itself.

---

# Post-Launch: Announcements, Mailbox & Calendar

A new, fully additive feature set — nothing above this was touched to
build it.

## Announcements / Mailbox

One-way messages with attachments (PDF, Word, or images, up to 10MB
each, up to 5 per message):

- **Admin** can send to any combination of classes, or everyone at
  once (`sentToAll`) — reaching that class's teacher(s) *and* every
  student in it.
- **Teacher** can only ever send to the students in their own class —
  enforced server-side in `announcement.controller.js`, not just left
  out of the UI. A teacher's message never reaches other teachers.
- The actual recipient list is computed and stored once, at send time
  (`AnnouncementRecipient`) — this is what makes "seen by 12 of 15"
  accurate and keeps a sent message's audience historically correct
  even if a student later changes class.
- **Permanently deleted after 3 months** — not archived. Cleanup runs
  lazily (`announcementCleanup.service.js`) at the top of every
  mailbox/announcement read, and every read query also filters out
  anything past `expiresAt` regardless, so an expired message is never
  visible even in the brief window before cleanup next runs.
- Email is a best-effort "you have a new message" notification only,
  sent with a short pause between each recipient — the real message
  is always already sitting in the in-app mailbox regardless of
  whether the email succeeds, gets delayed by Brevo's daily cap, or
  fails outright.

## Calendar

One shared, school-wide calendar — Admin adds/removes events by
tapping a date; Teacher and Student both view the same calendar
read-only. Today's event(s), if any, show as an in-app reminder banner
on every dashboard the moment someone opens the app — this is the
"in-app" half of the reminder system; true push notifications (arriving
even when the app isn't open) were intentionally deferred as a
follow-up, since they need explicit per-person opt-in and are a
meaningfully bigger, separate piece of work.

## Storage

Attachments are stored the same way as everything else added
post-launch — as bytes directly in the database, not on disk — for the
same reason: most free hosts wipe local disk on every restart.

---

# Post-Launch: Spreadsheet Roster Management & Deletion

A teacher couldn't fix a forgotten student name, delete an abandoned
spreadsheet, or start over for a class/session/term they'd already
begun — three related gaps with one common cause: once created, a
spreadsheet's student list and existence were both effectively frozen.

- **Add/remove students on an unlocked version** — `POST` and `DELETE`
  on `/teacher/spreadsheet-versions/:versionId/students`. Removing a
  student also cleans up any scores/psychomotor/attendance/remarks
  already entered for them on that version, so nothing orphaned is left
  behind. Both are blocked the moment a version is `LOCKED`, same as
  every other edit.
- **Delete an entire spreadsheet** — `DELETE
  /teacher/spreadsheets/:spreadsheetId` — but only if it has **never**
  once reached `LOCKED` status, checked server-side against every
  version's history, not just its current one. This is the same
  historical-integrity rule that governs everything else in this
  system: there is no code path that can destroy an official, once-
  locked academic record, no matter how the request is framed.
- Hitting the "a spreadsheet for this class/session/term already
  exists" conflict now tells the teacher they can delete the old draft
  (if it's still eligible) and try again, instead of leaving them stuck.

---

# Post-Launch: Subject Management on Unlocked Spreadsheets

Same gap as the student-roster fix, applied to subjects: a teacher who
forgot a subject when first creating a spreadsheet had no way to add
one afterward, short of the (mostly unusable, since deletion is blocked
once locked) option of abandoning the whole thing.

- **Add subjects** — `POST /teacher/spreadsheet-versions/:versionId/subjects`,
  picking from the existing global subject list or creating a brand new
  one on the fly (reusing the same subject-creation endpoint the
  create-spreadsheet wizard already uses).
- **Remove a subject** — `DELETE .../subjects/:subjectId` — also
  deletes every student's score already entered for it on this version,
  so nothing is left orphaned.
- Both blocked the instant a version is `LOCKED`, identical to every
  other edit rule in this system.

In the editor, both live next to the existing student-roster controls:
an "➕ Add Subject" button, and a small "✕" on each subject's column
header.
